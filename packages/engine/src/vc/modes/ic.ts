...
export async function runIC(ctx: RunContext, io: EngineIO): Promise<ICResult> {
  let seq = 0;
  const emit = async (actor: string, name: string, segment: string, content: string, fields?: unknown, avatar?: string) => {
    await io.emit({ type: "turn.completed", id: randomUUID(), actor, actorName: name, avatar, segment, seq: seq++, content, fields });
  };
...
will_champion = are you willing to stake your name and make the case FOR investing. Write prose in the language of the BP.`,
      ...
      await emit(persona.id, persona.name, "S1", `Stance: ${out.stance} · conviction ${out.conviction}${out.will_champion ? " · will champion" : ""}\n${out.reason}${out.is_fatal ? `\n⚠️ Fatal: ${out.fatal_reason}` : ""}`, out, persona.avatar);
  ...
  if (!champion) {
    await emit("host", "Chair", "S2", "No one is willing to champion this. The committee passes — no meeting needed.");
  ...
  await emit(champion.persona.id, champion.persona.name, "S3", championCase.case_for, { role: "champion" }, champion.persona.avatar);
  ...
  await emit(dissenter.persona.id, dissenter.persona.name, "S5", dissent.kill_case, { role: "dissenter", candidate_fatal: dissent.candidate_fatal }, dissenter.persona.avatar);
  ...
  await emit("host", "Chair", "S6", `Crux: ${cruxText}`, cruxOut);
  ...
  for (let r = 0; r < CAPS.ic_attack_rounds; r++) {
    ...
    await emit(champion.persona.id, champion.persona.name, "S7", champRb.rebuttal, { round: r + 1, role: "champion" }, champion.persona.avatar);
    ...
    await emit(dissenter.persona.id, dissenter.persona.name, "S7", disRb.rebuttal, { round: r + 1, role: "dissenter" }, dissenter.persona.avatar);
  ...
  await emit("host", "Chair", "S8", `Fatal resolved: ${judgment.fatal_resolved ? "yes" : "no"} · Crux resolved: ${judgment.crux_resolved ? "yes" : "no"}`, judgment);
